...
class Regform(forms.Form):
    username = forms.CharField(label='用户名', max_length=15, min_length=2,
    ...
    password_again = forms.CharField(label='再输入一次密码', min_length=8, max_length=15, widget=forms.PasswordInput(
        attrs={'class': 'form-control', 'placeholder': '再输入一次密码'}))
    verification_code = forms.CharField(label='验证码', required=False,
                                        widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '发送验证码'}))
...
class ChangePasswordForm(forms.Form):
    ...
    new_password_again = forms.CharField(label='再次输入', widget=forms.PasswordInput(
    ...
    def clean(self):
        ...
        if new_password != new_password_again or new_password == '':
            raise forms.ValidationError('请检查你的密码')
